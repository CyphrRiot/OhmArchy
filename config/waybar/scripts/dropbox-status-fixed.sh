...
# Get dropbox status - filter out extension loading messages
STATUS=$(dropbox status 2>&1 | grep -v "load fq extension" | grep -v "UserWarning")
...
if [[ $STATUS == *"isn't running"* ]]; then
    ...
    FILE_STATUS=$(dropbox filestatus 2>&1 | grep -v "load fq extension" | grep -v "UserWarning")
    ...
    # Default status for when we can't determine actual state
    echo '{"text": "Running", "tooltip": "Dropbox is running", "class": "running", "alt": "running"}'
fi